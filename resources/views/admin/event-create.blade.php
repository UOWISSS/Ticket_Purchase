...
@section('title', 'Esemény létrehozása')
...
<div class="container py-4">
    ...
    <form method="POST" action="{{ route('admin.events.store') }}">
        ...
        <div class="mb-3">
            ...
            <input type="datetime-local" class="form-control @error('event_date_at') is-invalid @enderror" id="event_date_at" name="event_date_at" required value="{{ old('event_date_at') }}">
            @error('event_date_at')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="datetime-local" class="form-control @error('sale_start_at') is-invalid @enderror" id="sale_start_at" name="sale_start_at" required value="{{ old('sale_start_at') }}">
            @error('sale_start_at')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="datetime-local" class="form-control @error('sale_end_at') is-invalid @enderror" id="sale_end_at" name="sale_end_at" required value="{{ old('sale_end_at') }}">
            @error('sale_end_at')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="number" class="form-control @error('max_number_allowed') is-invalid @enderror" id="max_number_allowed" name="max_number_allowed" min="1" required value="{{ old('max_number_allowed', 1) }}">
            @error('max_number_allowed')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3 form-check">
            <input type="checkbox" class="form-check-input" id="is_dynamic_price" name="is_dynamic_price" value="1" {{ old('is_dynamic_price') ? 'checked' : '' }}>
